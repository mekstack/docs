...
Если нужно что-то добавить, то открываем нужный .md файл, редактируем его в синтаксисе MarkDown и создаем Pull Request. После, в комментарии к вашему Pull Request'у появится ссылка на получившуюся документацию вида `pr-#.docs.staging.mekstack.ru` где #-номер вашего Pull Request'a. После ревизии и merg'a владельцем изменения появятся на `docs.staging.mekstack.ru`.
